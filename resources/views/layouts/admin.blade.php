                        ...
                        @else
                            <li class="nav-item dropdown">
                                <a id="navbarDropdown" class="nav-link dropdown-toggle" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>
                                    {{ Auth::guard('destination')->user()->name }} <span class="caret"></span>
                                </a>
        ...
        <main class="py-4">
          <div class="row">
            <div class="col-12 col-sm-12 col-md-3 col-lg-3 px-5">
              <div class="card">
                            @if (Auth::guard('destination')->user()->genre == 'male')
                                <img class="card-img" src="img/man.svg" style="width: 85%;" alt="Bologna">
                            @endif
                            @if (Auth::guard('destination')->user()->genre == 'female')
                                <img class="card-img" src="img/woman.svg" style="width: 85%;" alt="Bologna">
                            @endif
                
                <div class="card-img-overlay" style="height: 30px;">
                  <a href="{{ route('home') }}" class="btn btn-light btn-sm"><b>Matricule: {{ Auth::guard('destination')->user()->matricule }}</b></a>
                </div>
                <div class="card-body">
                  <h4 class="card-title">{{ Auth::guard('destination')->user()->name . Auth::guard('destination')->user()->subname }}</h4>
                  ...
                  <a href="{{ route('logout-admin') }}" class="btn btn-danger m-1" style="width: 100%;" onclick="event.preventDefault(); document.getElementById('logout-form').submit();"> Deconnexion </a>
                </div>
